...
class DetalheCarteira extends TPage {
    private $form;
    private static $database = 'api';
    private static $activeRecord = 'Carteira';
    private static $primaryKey = 'id';
    private static $formName = 'DetalheCarteira';
    ...
    public function __construct($param) {
        ...
        $userid = TSession::getValue('userid');
         $criteria_carteira->add(new TFilter('responsavel_id', '=', $userid));
    ...
    public static function onChangeDropDown($param) {
        if (isset($param['carteira'])) {
            ...
            $action = new TAction(['Mestre_DetalheTeste', 'onReload']);
            $action->setParameter('responsavel_id', $param['carteira']);
            TApplication::loadPage('Mestre_DetalheTeste', 'onReload', $action->getParameters());
        }
    }
}
