...
try {
    ...
    $carteira=$_POST['nome_carteira'];
      $insertStmt = $conn->prepare("INSERT INTO carteira( responsavel_id, flag_ativo, nome) VALUES (2,1,?)");
        $insertStmt->execute([$carteira]);
        echo '<div style="padding: 15px; background-color: #d4edda; color: #155724; border: 1px solid #c3e6cb; border-radius: 5px; margin-top: 20px;">
                <strong>Carteira Inserida com Sucesso!</strong><br>
              </div>';
     }
 ...
 $dadosBulk = json_encode(array("user"=>"alextelocken@gmail.com","psw"=>"hivros@qujrop@2jezDo","id_lote_consulta"=>"$id_consulta"));
...
try {
    ...
    echo 'Erro de conexão: ' . $e->getMessage();
            ...
            $updateStmt = $conn->prepare("UPDATE consulta_api SET flag_utilizado = 1 WHERE id_lote_consulta = ?");
